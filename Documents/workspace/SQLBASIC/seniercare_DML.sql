...
### 로그인 ###
## 최종 사용자(아이디, 비밀번호)
-- 요양사 테이블에 입력한 아이디와 비밀번호에 
-- 해당하는 레코드가 존재하면 로그인 성공
-- 추가적인 데이터 x
...
-- 요양사 테이블에 레코드(아이디와 암호화된 비밀번호, 전화번호, 가입경로, 이름)를 생성
...
SELECT * FROM nurse WHERE tel_number = '01012345678';
-- 레코드 삽입시 이미 인증 절차를 가지고 있는 전화번호인지 확인
...
### 인증번호 확인 ###
## 최종사용자(인증번호)
-- 전화번호 인증테이블에서 전화번호에 해당하는 
-- 인증번호가 사용자가 입력한 인증번호와 같은지 비교
-- 필요한 추가 데이터 : 전화번호 (화면에서 가져옴)
SELECT * FROM tel_auth WHERE tel_number = '01012345678' AND auth_number = '0123' ;


### 전체 고객 수 및 전체 페이지 수 불러오기 ###
## 최종사용자() => (전체 고객수 , 전체 페이지 수)
-- 고객 테이블에서 전체 레코드의 갯수와  전체 레코드의 갯수에 10을 나눈 값에 소숫점 첫번째 자리에서 올림 하여 나타냄
SELECT COUNT(*), CEIL((COUNT(*) / 10)) FROM customer;

### 전체 고객 리스트 불러오기 ###
## 최종사용자(페이지 번호) => (고객번호, 고객명, 나이, 지역, 담당자 이름)
...
### 고객 삭제 ###
## 최종 사용자(고객 정보) 
-- 고객 테이블에서 지정한 고객(고객번호에 해당하는)의 레코드를 삭제하는 작업
...
### 고객등록 ###
## 최종사용자(고객사진, 고객이름, 생년월일, 담당자, 주소)
-- 고객 테이블의 레코드(고객 사진, 고객이름, 생년월일, 담당자, 주소)를 삽입 
-- 필요추가 데이터 : 지역(화면)
...
### 고객 상세보기 ###
## 최종사용자(고객 정보) => (고객 번호, 고객 사진, 고객 이름, 생년월일, 담당자 이름)
-- 고객 테이블에서 특정 고객을 조회하여 반환, 담당자 이름을 반환하기위해 요양사 테이블을 조인
-- 필요한 추가 데이터 : 고객번호(화면에서)
...
### 관리 기록 리스트 ###
## 최종사용자(고객정보) => (날짜, 내용, 사용용품, 갯수)    
-- 관리 기록 테이블에서 특정 고객에 대한 관리 기록 리스트를 날짜 순으로 내림차순 정렬하여 조회
-- 추가 필요 데이터 : 고객번호(화면에서)
SELECT CM.manage_date '관리일', CM.comment '관리내용', G.name '영품이름', CM.used_goods_count '사용 갯수'  
...
### 관리 기록 ###
## 최종사용자(내용, 사용용품, 용품갯수)
-- 관리기록테이블에 레코드(고객번호, 날짜, 내용, 사용용품, 용품갯수)를 삽입
-- 필요한 추가데이터 : 고객번호(화면상에서), 날짜(서버에서) 
...
UPDATE goods SET goods_count = goods_count - 2 WHERE goods_number = 1;

### 고객 수정 ###
## 최종사용자(고객 사진, 고객 이름, 생년월일 ,담당자, 주소)
-- 고객테이블에서 특정 고객에 대한 레코드(고객 사진, 고객 이름, 생년월일 ,담당자, 주소)를 수정
-- 필요한 추가 데이터 : 고객번호(화면상에서)

-- 수정 하려고 하는 레코드가 존재하는지 확인
SELECT * FROM customer WHERE customer_number = 1;
-- 레코드 수정시 담당자 관계 확인
