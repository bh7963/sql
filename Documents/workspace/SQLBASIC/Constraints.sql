...
-- 1. NOT NULL 제약조건 : 해당 컬럼에 NULL값을 포함하지 못하도록 제약
...
-- 2. UNIQUE 제약조건 : 해당 컬럼에 중복된 데이터를 포함할 수 없도록 하는 제약
-- (자기 자신 테이블 INSERT, UPDATE)  
...
-- UNIQUE로 지정된 컬럼은 중복된 데이터로 수정 할 수 없음
-- UNIQUE로 지정
...
-- FOREIGN KEY제약조건이 적용된 컬럼에는 참조하고 있는 테이블의 컬럼에 값이 존재하지 않으면 삽입, 수정이 불가능
INSERT INTO foreign_key_table VALUES (1,0);
INSERT INTO foreign_key_table VALUES (1,1);

-- FOREIGN KEY제약조건으로 참조되어지고 있는 테이블의 레코드는 수정, 삭제가 불가능
UPDATE foreign_key_table SET foreign1 = 2 WHERE primary1 = 1;
UPDATE key_table SET primary_column = 2 WHERE primary_column = 1;
DELETE FROM key_table WHERE primary_column = 1;

-- FOREIGN KEY제약조건으로 참조되어지고 있는 테이블의 컬럼 변경 작업이 불가능 
...
-- RESTRICT : 참조되고 있는 테이블의 데이터가 삭제 또는 불가능하게 함
...
CREATE TABLE chek_table (
	primary_column INT PRIMARY KEY,
    check_column VARCHAR(5) CHECK(check_column IN ('남', '여'))
);
-- CHECK 제약조건이 걸린 컬럼에 조건에 해당하지 않는 값을 삽입, 수정 할 수 없음
INSERT INTO chek_table VALUES (1, '남');
INSERT INTO chek_table VALUES (2, '남자');
UPDATE chek_table SET check_column = '여자';
