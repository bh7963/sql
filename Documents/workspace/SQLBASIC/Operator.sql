...
INSERT INTO transaction VALUES (1, '2024-07-10', '구매', '기타자재', 100000, 0, null);
INSERT INTO transaction VALUES (2, '2024-07-10', '판매', '영양제', 700000, 70000, '종합 비타민');
INSERT INTO transaction VALUES (3, '2024-07-12', '판매', '영양제', 1200000, 120000, '종합 비타민');
...
-- 산술 연산자
-- +, -, *, /, %
SELECT amount + tex, breakdown from transaction;

-- 비교 연산자 (WHERE절에서 자주 사용)
-- 원하는 레코드를 정확히 조회하는데 중요한 역할을 함
-- = : 조항이 우항과 같으면 truetansaction_type
SELECT * FROM transaction WHERE tansaction_type = '구매';
...
-- 논리 연산자
-- AND(&&) : 좌항과 우항 모두 true이면 true
SELECT * FROM transaction WHERE tansaction_type = '판매' AND amount >= 1500000;

-- OR(||) : 좌항이나 우항이 true 이면 true
SELECT * FROM transaction WHERE transaction_date >= '2024-07-15' OR tansaction_type = '판매';

-- XOR : 좌항과 우항이 서로 다르면 true 
SELECT * FROM transaction WHERE transaction_date >= '2024-07-15' XOR tansaction_type = '판매';
...
SELECT * FROM transaction ORDER BY tex, amount DESC;
