...
SELECT date, IF(amount > 100000, '고액', '저액') '금액' 
FROM sale;

-- IFNULL 함수 : 첫번째 매개변수의 값이 NULL인지에 따라 결과를 반환하는 함수
-- IFNULL (값, 값이 null일때 반환할 결과)
SELECT 사번, 이름, IFNULL(부서명, '없음') FROM employee_view;
...
-- 					   매개변수중 하나라도 null이면 null을 반환
SELECT CONCAT(이름, " / " ,부서명) '결합' FROM employee_view;

-- LEFT(문자열, 길이), RIGHT(문자열, 길이) : 문자열을 왼쪽, 오른쪽에서부터 추출
SELECT LEFT(이름, 2), RIGHT(이름, 2) FROM employee_view;

-- REPLACE(기준 문자열, 찾을 문자열, 바꿀 문자열) : 특정 문자열을 지정한 문자열로 변경하는 함수
SELECT REPLACE(이름, '길동', '순자')'이름변경' FROM employee_view; 